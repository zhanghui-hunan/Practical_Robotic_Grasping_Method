...
class DopeNetwork(nn.Module):
    def __init__(
            ...
            numBeliefMap=9,
            numAffinity=16,
            stop_at_stage=4  # number of stages to process (if less than total number of stages)
        ):
        global block
        block = BasicBlock
        global layers
        ...
        self.vgg = nn.Sequential(OrderedDict([
            ('0', nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=3, bias=False)),
            ('1', nn.BatchNorm2d(64)),
            ('2', nn.ReLU(inplace=True)),
            ('3', nn.MaxPool2d(kernel_size=3, stride=2, padding=0)),
            ##block
            ('4', self._make_layer(block, 64, layers[0], stride=1)),
            ('5', self._make_layer(block, 128, layers[1], stride=1)),
            ('6', self._make_layer(block, 256, layers[2], stride=2)),
            ('7', self._make_layer(block, 512, layers[3], stride=1))
        ]))
        for i_layer in range(8):
            self.vgg.add_module(str(i_layer), self.vgg[i_layer])

        # Add some layers
        i_layer = 7
        self.vgg.add_module(str(i_layer), nn.Conv2d(256, 128, kernel_size=3, stride=1, padding=1))
