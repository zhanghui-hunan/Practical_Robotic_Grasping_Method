...
from cuboid import *
#from detector_vgg19 import *
#from detector_vgg16 import *
#from detector_FastNet-V2 import *
from detector_FastNet-V1 import *
...
def DrawCube(points, color=(255, 0, 0)):
    ...
    # Axis calculation
    x_left = (points[3][0] + points[4][0]) / 2
    y_left = (points[3][1] + points[4][1]) / 2

    x_right = (points[2][0] + points[5][0]) / 2
    y_right = (points[2][1] + points[5][1]) / 2

    x_top = (points[0][0] + points[5][0]) / 2
    y_top = (points[0][1] + points[5][1]) / 2

    x_down = (points[3][0] + points[6][0]) / 2
    y_down = (points[3][1] + points[6][1]) / 2

    x_center = (x_left + x_right) / 2
    y_center = (y_top + y_down) / 2

    print(",%f,%f" % (x_center, y_center))

    x_front = (points[0][0] + points[2][0]) / 2
    y_front = (points[0][1] + points[2][1]) / 2
    ...
    # x-red
    DrawLine((x_left, y_left), (x_center, y_center), (255, 0, 0), 5)

    # y-green
    DrawLine((x_down, y_down), (x_center, y_center), (0, 255, 0), 5)

    # z-blue
    DrawLine((x_front, y_front), (x_center, y_center), (0, 0, 255), 5)
...
def run_dope_node(params, freq=5):
    ...
    #publish pose 7 value
    pub = rospy.Publisher('pose_pos_ori', String, queue_size=10)
        ...
        if g_img is not None:
            ...
            for m in models:
                ...
                for i_r, result in enumerate(results):
                    ...
                    #########################
                    # output translation X,Y,Z to_topic:translation
                    x = msg.pose.position.x
                    y = msg.pose.position.y
                    z = msg.pose.position.z

                    # translation = ("%f,%f,%f" % (x, y, z))
                    # talkerT(translation)

                    # output rotate rx,ry,rz to_topic:rotate
                    ...
                    a = [rx, ry, rz, rw]
                    q = tf.transformations.euler_from_quaternion(a)
                    c = q[1] + 1.57   
                    b = tf.transformations.quaternion_from_euler(q[0], c, q[2])

                    # quaternion_to_Euler-angle
                    #rx = math.atan2(2*(x1*w1 + y1*z1), 1-2*((x1)**2 + (y1)**2))
                    #ry = math.asin(2*(w1*y1 - z1*x1))
                    #rz = math.atan2(2*(w1*x1+x1*y1), 1-2*(y1**2 + z1**2))

                    # pose_pos_ori = ("%f,%f,%f,%f,%f,%f,%f" % (x, y, z, b[0], b[1], b[2], b[3]))
                    #pose_pos_ori = ("x:%f,y:%f,z:%f,rx:%f,ry:%f,rz:%f,rw:%f" % (x, y, z, rx, ry, rz, rw))
                    pose_pos_ori = ("%f,%f,%f,%f,%f,%f,%f" % (x, y, z, rx, ry, rz, rw))
                    rospy.loginfo(pose_pos_ori)
                    pub.publish(pose_pos_ori)
                    ##########################
